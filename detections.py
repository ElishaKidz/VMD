...
class DetectionsFromContours:
    ...
    def find_detections_from_contours(self, contours,image_size=None):
        records = []
        for contour in contours:
            bbox = cv.boundingRect(contour)  # x,y,w,h
            # convert the bounding box to the requested format by the user
            bbox = pbx.convert_bbox(bbox, from_type='coco', to_type=self.bbox_format, image_size=image_size)
    ...
    def __call__(self, binary_map):
        ...
        return self.find_detections_from_contours(contours, binary_map.shape[::-1])
