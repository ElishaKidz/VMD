...
import numpy as np
import cv2
from MovingCameraVMD.ForegroundPicker import ForegroundPicker
...
mcd = ForegroundPicker()
os.makedirs("ForegroundPicker_results", exist_ok=True)
isFirst = True
...
for i in tqdm(range(200)):
    ...
    if (isFirst):
        isFirst = False
        out = cv2.VideoWriter('ForegroundPicker_results/tel_move.avi', cv2.VideoWriter_fourcc(*'DIVX'), 25, gray.shape[::-1])
    mask = mcd.pick_foreground(gray)
